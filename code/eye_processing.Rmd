...
get_pid <- function(path) str_extract(basename(path), "^\\d{5}")
...
process_edf <- function(path) {
  ...
  # Join key (only start). We don't need an 'end' for msg.
  msg[, start := msg_time]
  setkey(msg, start)
  ...
  # --- rolling joins (same order as working single-file code) ---
  # Label saccades by most recent prior message, then label fixations from that.
  sac_labeled <- msg[sac,  on = .(start), roll = TRUE]
  labeled     <- sac_labeled[fix, on = .(start), roll = TRUE]

  # keep only rows that inherited a message label
  out <- labeled[!is.na(msg_time)]

  # attach pid (keep as character to preserve leading zeros)
  out[, pid := pid]

  # create trial_index from trial_num and drop trial_num if you don't need it
  if (!"trial_num" %in% names(out)) out[, trial_num := NA_integer_]
  out[, trial_index := trial_num][, trial_num := NULL]

  # optional tidy: drop helper columns you don't need from joins
  # (keep timing columns if they're useful for later overlap logic)
  # Here we keep them.

  return(out[])
}

# --- run over all EDFs and combine ---
data_eye_processed <- rbindlist(lapply(edf_files, process_edf), fill = TRUE)

# drop msg_time if you don't need it downstream
if ("msg_time" %in% names(data_eye_processed)) {
  data_eye_processed[, msg_time := NULL]
}

# ensure trial_index exists (safety) and reorder columns
if (!"trial_index" %in% names(data_eye_processed)) {
  data_eye_processed[, trial_index := NA_integer_]
}
