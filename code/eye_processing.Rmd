...
```{r}
# get a list of all EDF files
edf_files <- list.files("/Users/anhthudang/Downloads/Piloting Eyetracking/regret_eyetracking/experiment/data", pattern = "\\.EDF$", full.names = TRUE)
...
# function to process the files one by one, just like the single-participant code above but with a few additions
process_edf <- function(path) {
  pid <- get_pid(path) # getting participant id
  ...
  # joining all in one go here because of errors
  labeled <- fix[sac[msg, roll = TRUE], roll = TRUE]

  sac_fix_labeled <- sac_fix_labeled |>
    filter(!is.na(msg_time)) |>
    mutate(pid = pid)

  return(sac_fix_labeled)
}

# apply the function the all edf files
data_eye_processed <- rbindlist(lapply(edf_files, process_edf), fill = TRUE) |>
  rename(trial_index = trial_num) |>
  relocate(pid, .before = trial_index) |>
  select(-msg_time) |>
