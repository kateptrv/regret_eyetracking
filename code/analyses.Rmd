...
<<<<<<< HEAD
  dplyr::filter(pid != 33928 & pid != 34108 & pid != 33835)
=======
  dplyr::filter(pid != 33928 & pid != 33706 & pid != 34108 & pid != 33835) # excluding based on session notes
>>>>>>> 39afc365c96ee60392c61c32ac10a75ee11a88f0
...
data %>%
  dplyr::filter(type == "choice") %>%
  dplyr::filter(fix_duration > 200) |>
...
```{r}
trial_summ <- data_aoi %>%
  dplyr::filter(type == "feedback", aoi_tree %in% c("tree_1","tree_2","tree_3")) %>%
  ...
  # sum fixation durations per trial × aoi_rank
...
per_tree <- data_aoi %>%
  ...
  group_by(pid, trial_index, aoi_rank) %>%
  mutate(dwell = sum(fix_duration, na.rm = TRUE)) %>%
  ungroup() |>
  group_by(pid, trial_index) %>%
  mutate(
  ...
  left_join(trial_next %>% select(pid, trial_index, next_choice),
            by = c("pid","trial_index")) %>%
  dplyr::filter(!is.na(next_choice), !is.na(prop_dwell)) %>%
...
```{r}
data_beh <- data |>
  select(pid, condition, trial_index, choice_prob, points, regret, totalpoints, choice_goodbad) |>
  distinct()

data_short <- data_beh |>
  group_by(pid) |>
  mutate(total_expected_value = sum(choice_prob),
         avg_regret = mean(regret, na.rm = TRUE)) |>
  ungroup() |>
  select(pid, condition, total_expected_value, avg_regret) |>
  distinct()
```
...
```{r}
data_beh |>
  ggplot(aes(x = trial_index, y = regret, color = condition)) +
  geom_smooth(method = "loess", span = 0.25) +
  labs(x = "Trial Index",
       y = "Subjective regret")

data_beh |>
  ggplot(aes(x = choice_goodbad, y = regret)) +
...
data |>
  dplyr::filter(type == "choice") |>
  ggplot(aes(fix_duration, regret)) +
  #geom_smooth(method = "lm", color = "black", size = 1) +
...
data |>
  dplyr::filter(type == "feedback") |>
  ggplot(aes(regret, fix_duration)) +
  geom_smooth(method = "lm", color = "black", size = 1) +
  # stat_dots(
  #   side = "right",
  #   justification = 0,
...
data |>
  dplyr::filter(type == "choice") |>
  ggplot(aes(x = regret_rt, y = regret)) +
  geom_smooth(method = "lm", color = "black", size = 1) +
  geom_point(aes(color = regret), alpha = 0.5) +
  labs(x = "Regret rt", y = "Regret") +
  theme(legend.position = "none") +
  theme(panel.grid.major.x = element_blank(),
        panel.grid.minor.x = element_blank())
```
